...
tld_dns_table = {
    "com": ("127.0.0.1", 1700),  # Authoritative server for .com
    "org": ("127.0.0.1", 1800),  # Authoritative server for .org
}
...
def handle_client(client_data, client_address, server_socket):
    try:
        ...
        print(f"[TLD] Received query for: {data} from {client_address}")

        # Extract domain and TLD
        domain_parts = data.split(".")
        tld = domain_parts[-1]

        # Get authoritative server for the TLD
...
def start_server():
    ...
    server_socket.bind((HOST, TLD_PORT))  # Bind to TLD DNS server port
